...
/**
 ...
 This category adds class methods to `UIImage` to create an animated `UIImage` from a GIF.
 */
@interface UIImage (animatedGIF)

/*
 UIImage *animation = [UIImage animatedImageWithGIFData:data];
 
 I interpret `data` as a GIF.  I create an animated `UIImage` using the source images in the GIF.
...
/*
 UIImage *image = [UIImage animatedImageWithGIFUrl:url];
 
 I interpret the contents of `url` as a GIF.  I create an animated `UIImage` using the source images in the GIF.
 
 I operate exactly like `+[UIImage animatedImageWithGIFData:]`, except that I read the data from `url`.  If `url` is not a `file:` URL, you probably want to call me on a background thread or GCD queue to avoid blocking the main thread.
 */
